...
def main():
    ...
    with st.sidebar:
        ...
        # Intento de cargar desde st.secrets (opcional)
        token_sugerido = ""
        try:
        ...
        token = st.text_input("Bearer Token", value=(token_sugerido if usar_secret else ""), type="password", help="Pega el token que te dio Afacturar. No incluyas 'Bearer', la app lo añade.")
    ...
    nota_libre = st.text_area("nota (opcional, string con comillas simples)", "{'MOTIVO':'Ajuste','OBS':'Prueba integración'}", height=80)
    ...
    st.markdown("### 3) Servicio y detalle mínimo")
    ...
    if colBtn1.button("🧩 Construir JSON"):
        try:
            ...
            detalle = [{
                ...
                "cargo_descuento": {"es_descuento": True, "porcentaje_cargo_descuento": _fmt_dec("0.00"), "valor_base_cargo_descuento": _fmt_dec("0"), "valor_cargo_descuento": _fmt_dec("0")},
                "impuestos_detalle": {"codigo_impuesto":"0","porcentaje_impuesto":_fmt_dec("0.00"),"valor_base_impuesto":_fmt_dec("0"),"valor_impuesto":_fmt_dec("0")},
                "retenciones_detalle":[{"codigo":"0","porcentaje":_fmt_dec("0.00"),"valor_base":_fmt_dec("0"),"valor_retenido":_fmt_dec("0")}],
                "valores_unitarios":{"valor_impuesto_1":_fmt_dec("0"),"valor_impuesto_2":_fmt_dec("0"),"valor_impuesto_3":_fmt_dec("0"),"valor_impuesto_4":_fmt_dec("0"),"valor_a_pagar":_fmt_dec(total)},
                ...
                "datos_remesa":{"tipo_servicio":"0","numero_radicado_de_la_remesa":"N/A","numero_de_remesa":"N/A","valor_flete":"0","cantidad_transportada":"0","unidad_medida":"KGM"}
            }]
            impuestos = [{"codigo_impuesto":"0","porcentaje_impuesto":_fmt_dec("0.00"),"valor_base_calculo_impuesto":_fmt_dec("0"),"valor_total_impuesto":_fmt_dec("0")}]
            descuentos = [{"codigo_descuento":"99","porcentaje_descuento":_fmt_dec("0.00"),"valor_base_calculo_descuento":_fmt_dec("0"),"valor_total_descuento":_fmt_dec("0")}]
            valor_nc = {
            ...
            generalidades = {
                "tipo_ambiente_dian": tipo_amb,
                "version": "1",
                "identificador_transmision": f"PKG-{id_nc}",
                "rg_tipo": "PDF",        # o HTML/PDF_PROPIO si lo usas
                "rg_base_64": "",        # opcional
    ...
    if colBtn2.button("⬇️ Descargar JSON"):
        if "payload" not in st.session_state:
            ...
            st.download_button("Descargar archivo", data=data, file_name=f"{id_nc}_nota_credito.json", mime="application/json", use_container_width=True)
    ...
    if colBtn3.button("🚀 Enviar a Afacturar"):
        try:
            ...
            try:
                ...
                st.code(r.text)
            if r.status_code in (401, 403) or "Unauthenticated" in r.text:
                st.warning("Respuesta de autenticación fallida. Verifica:\n- Token pertenece al mismo ambiente seleccionado.\n- 'documento_obligado' corresponde al NIT asociado al token.\n- Prefijo 'Bearer ' correcto (la app lo agrega).")
        except Exception:
